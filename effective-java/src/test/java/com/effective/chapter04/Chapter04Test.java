...
public class Chapter04Test {
    ...
    @Test
    public void test21() {
        // 类实现接口之后，接口就可以充当引用这个类实例的类型
        // 常量接口模式是对接口的不良使用

        // 标签类过于冗长，容易出错，效率低下，是对类层次的一种简单效仿

        // 嵌套类：静态成员类，非静态成员类，匿名类，局部类
        // 静态成员类：可以访问外围类的私有成员，常作为外围类的辅助类
        // 非静态成员类：拥有外部类的引用，可以通过this获取外围实例的引用
        // 匿名类：不能执行 instanceof，无法继承实现接口(不能包含静态成员)
        // 局部类：任何 “可以声明局部变量的地方” 都可以 声明局部类

        // 如果声明成员类不需要访问外围实例，始终声明成静态的(非静态内部类会包含一个指向外围类的引用(浪费时间空间，可能造成内存泄漏))
    }
